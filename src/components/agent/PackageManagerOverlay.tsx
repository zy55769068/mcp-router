...
const PackageManagerOverlay: React.FC<PackageManagerOverlayProps> = ({ isOpen, onInstallComplete, onCancel }) => {
    const { t, i18n } = useTranslation();
    const [packageManagers, setPackageManagers] = useState<{node: boolean, pnpm: boolean, uv: boolean}>({node: false, pnpm: false, uv: false});
    ...
    const incrementDisplayCount = useCallback(async () => {
        try {
            const result = await window.electronAPI.incrementPackageManagerOverlayCount();
            if (result.success) {
                setDisplayCount(result.count);
            }
        } catch (error) {
            console.error('Failed to increment package manager overlay display count:', error);
        }
    }, []);
    ...
    const handleInstallPackageManagers = async () => {
        try {
            ...
            if (result.success) {
                ...
                throw new Error(errors.join(', ') || 'Failed to install package managers');
            }
        } catch (error) {
            console.error('Failed to install package managers:', error);
            toast.error(error instanceof Error ? error.message : 'Failed to install package managers');
        } finally {
    ...
    const openDocumentation = () => {
        ...
        window.open(`https://docs.mcp-router.net/${isJapanese ? 'ja/' : ''}docs/agent/intro`, '_blank');
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={() => { /* Dialog stays open until installation is complete */ }}>
            <DialogContent className="sm:max-w-[500px] [&>button]:hidden">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-base mt-4">
                        {installationComplete ? 
                            t('agents.installationCompleteMessage', 'パッケージマネージャーのインストールが完了しました。') :
                            t('agents.packageManagerOverlayDescription', 'Agents require package managers to run MCP servers. The following package managers need to be installed:')}
                    </DialogDescription>
                </DialogHeader>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-green-700">
                                        {t('agents.restartInstructions', '環境変数を反映するためアプリを再起動してください。')}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-blue-700 mb-3">
                                        {t('agents.troubleshootingDescription', '複数回試行してもパッケージマネージャーのインストールが失敗し続ける場合は、代替インストール方法やトラブルシューティング手順についてドキュメントをご確認ください。')}
                                    </p>
                                    <Button
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        {!installationComplete && (
                            ...
                            >
                                ...
                                {isInstallingPM ? t('agents.installing') : t('agents.installPackageManagers')}
                            </Button>
                        )}
                        {installationComplete && (
                            <>
                                ...
                                <Button
                                    ...
                                    size="lg"
                                    className="flex-1"
                                    title={t('agents.restartDescription', '変更を反映するためにアプリケーションを再起動します')}
                                >
