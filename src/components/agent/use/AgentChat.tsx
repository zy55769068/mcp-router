...
const AgentChat: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setError(undefined);
    }, [agent?.id, agent?.instructions, setCurrentSessionId, setCurrentDeployedAgent, resetSessions, resetChatState]);
    ...
    }, [deleteChatSession, currentSessionId, agent?.id, authToken, fetchChatSessions]);
    ...
    useEffect(() => {
        const unsubscribeStart = window.electronAPI?.onChatStreamStart?.((data: any) => {
        ...
        const unsubscribeChunk = window.electronAPI?.onChatStreamChunk?.((data: any) => {
            ...
            if (data.agentId === agent?.id) {
                ...
                if (data.isToolInvocation && data.completeMessage && data.isCompleteMessage) {
                    ...
                    const hasToolResults = completeMessage.parts?.some(part => 
                    ...
                    // 完全なメッセージをstoreに追加
                    const existingIndex = messages.findIndex(msg => msg.id === completeMessage.id);
                    if (existingIndex >= 0) {
        ...
        const unsubscribeEnd = window.electronAPI?.onChatStreamEnd?.((data: any) => {
            ...
            if (data.agentId === agent?.id) {
                ...
                if (currentStreamMessage) {
                    const existingIndex = messages.findIndex(msg => msg.id === currentStreamMessage.id);
                    if (existingIndex >= 0) {
        ...
        const unsubscribeError = window.electronAPI?.onChatStreamError?.((data: any) => {
    ...
    }, [agent?.id, isStreaming, currentStreamMessage, currentSessionId, messages, addMessage, updateMessage, setCurrentStreamMessage, setIsStreaming]);
    ...
    useEffect(() => {
        const loadSessionMessages = async () => {
            if (selectedSession) {
                ...
                if (selectedSession.messages) {
                    ...
                    const sessionMessages = selectedSession.messages.filter(msg => msg.role !== 'system');
                    setMessages([
                    ...
                    try {
                        setIsLoading(true);
                        const fetchedMessages = await window.electronAPI.fetchSessionMessages(selectedSession.id);
                        
                        // Filter out system messages when setting from fetched messages
                        const sessionMessages = fetchedMessages.filter((msg: any) => msg.role !== 'system');
                        setMessages([
                            // Always include the system message
                            {
                                id: 'system-1',
                        ...
                        setError(error instanceof Error ? error : new Error('Failed to load session messages'));
                    } finally {
                        setIsLoading(false);
                    }
                }
        ...
        loadSessionMessages();
    }, [currentSessionId, selectedSession?.messages, selectedSession?.id, agent?.id, agent?.instructions, setMessages, resetChatState]);
