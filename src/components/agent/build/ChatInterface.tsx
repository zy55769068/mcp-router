...
import { Send, Check, X, AlertCircle, ChevronDown, ChevronUp, Square } from 'lucide-react';
...
const formatImageData = (data: string, mimeType?: string) => 
  data.startsWith('data:') ? data : `data:${mimeType || "image/png"};base64,${data}`;

const renderImage = (data: string, alt?: string, mimeType?: string, index?: number) => (
  <div className="space-y-2">
    <img 
      src={formatImageData(data, mimeType)} 
      alt={alt || `Tool result image${index !== undefined ? ` ${index + 1}` : ''}`}
...
const renderResultContent = (result: any) => {
    ...
    return (
      <div className="space-y-2">
        {result.content.map((item: any, index: number) => {
          if (item.type === 'image') {
            return <div key={index}>{renderImage(item.data, item.alt, item.mimeType, index)}</div>;
          }
          if (item.type === 'text') {
            return <pre key={index} className="whitespace-pre-wrap break-words text-sm">{item.text}</pre>;
          }
          return <pre key={index} className="whitespace-pre-wrap break-words text-sm">{JSON.stringify(item, null, 2)}</pre>;
        })}
      </div>
    );
  }
  ...
  return <pre className="whitespace-pre-wrap break-words text-sm">{JSON.stringify(result, null, 2)}</pre>;
};
    ...
    >
      <CollapsibleTrigger asChild>
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-muted/50 cursor-pointer">
          <div className={`text-sm flex items-center ${isError ? "text-red-600 font-medium" : "text-muted-foreground"}`}>
      ...
      <CollapsibleContent>
        <div className="space-y-3 mt-2">
          <div>
            <div className="text-xs text-muted-foreground mb-1">{t('agents.chat.query')}</div>
          ...
          <div>
            <div className={`text-xs mb-1 ${isError ? "text-red-600" : "text-muted-foreground"}`}>
              {isError ? t('agents.chat.error') : t('agents.chat.result')}
            </div>
            <div className={`p-3 rounded-md ${isError ? "bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800" : "bg-muted"}`}>
              {isError ? (
...
const ToolConfirmationCard: FC<{
  ...
  callId: string;
  toolName: string;
  onToolConfirmation: (toolCallId: string, toolName: string, args: any, confirmed: boolean) => void;
}> = React.memo(({ message, toolInvocation, callId, toolName, onToolConfirmation }) => {
  const { t } = useTranslation();
  ...
  return (
    <Card className="mb-4">
      <CardHeader>
        ...
        <CardDescription>{t('agents.chat.executeToolConfirm', { toolName })}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="bg-muted p-3 rounded-md">
          <pre className="whitespace-pre-wrap break-words text-sm">
...
const markdownComponents = {
  ...
  h3: ({ children }: any) => <h3 className="text-md font-medium mb-1 text-foreground">{children}</h3>,
...
const AssistantMessage: FC<{
  ...
  onToolConfirmation?: (toolCallId: string, toolName: string, args: any, confirmed: boolean) => void;
}> = React.memo(({ message, agent, openToolResults, onToggleToolResult, onToolConfirmation }) => {
  ...
  return (
    <div className="space-y-3">
      {message.parts.map((part, idx) => {
        if (part.type === 'text') {
          return (
            <div key={`${message.id}-part-${idx}`} className="w-full prose prose-sm max-w-none break-words">
              <ReactMarkdown
        ...
        if (part.type === 'tool-invocation') {
          ...
          if (state === 'call' && onToolConfirmation && !agent.autoExecuteTool) {
            return (
          ...
          if (state === 'result') {
            const isError = toolInvocation.result && typeof toolInvocation.result === 'object' && 'error' in toolInvocation.result;
            return (
...
const MessageDisplay: FC<MessageDisplayProps> = React.memo(({
  ...
  const { t } = useTranslation();
  const [openToolResults, setOpenToolResults] = useState<Record<string, boolean>>({});
  ...
  const toggleToolResult = useCallback((toolCallId: string) => {
    setOpenToolResults(prev => ({ ...prev, [toolCallId]: !prev[toolCallId] }));
  }, []);
              ...
              <Collapsible className="w-full">
                <CollapsibleTrigger asChild>
                  <div className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-md cursor-pointer">
                    <h3 className="font-medium">{t('agents.chat.availableTools')} ({enabledTools.length})</h3>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="sm" className="p-0 h-6 w-6">
    ...
    <div className="p-4 border-t bg-background">
      <form onSubmit={submitHandler} className="flex gap-2">
        ...
        {isLoading ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button type="button" size="icon" onClick={handleStopClick} variant="destructive">
