...
const ServerVariableDisplay: React.FC<ServerVariableDisplayProps> = ({ servers }) => {
    ...
    <div className="border-b mb-4 pb-4">
      <h3 className="font-medium mb-3 flex items-center">
        ...
        <span className="text-sm text-muted-foreground ml-2 font-normal">
          {serversWithVars.length}{t('agents.serverVariables.servers')} / {serversWithVars.reduce((count, server) => count + extractServerVariables(server).length, 0)}{t('agents.serverVariables.variables')}
        </span>
      </h3>
              ...
              <AccordionContent>
                <div className="grid gap-1 text-sm pt-2 pb-1">
                  {variables.map((variable) => (
                    <div key={variable.name} className="grid grid-cols-[1fr_2fr] gap-2 py-1 border-b border-dashed border-gray-200 dark:border-gray-700 last:border-0">
                      <div className="font-mono text-blue-600 dark:text-blue-400 flex items-center">
                        ...
                        <Badge variant="outline" className="ml-2 text-[10px] px-1 py-0 h-4">
                          {variable.source === 'env' ? 'ENV' :
                           variable.source === 'arg' ? 'ARG' : 'PARAM'}
                        </Badge>
                      </div>
                      <div className="text-gray-700 dark:text-gray-300 font-mono overflow-hidden text-ellipsis">
                        {variable.value || `<${t('agents.serverVariables.notSet')}>`}
