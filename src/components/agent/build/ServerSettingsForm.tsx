...
const ServerSettingsForm: React.FC<ServerSettingsFormProps> = ({
  ...
  const [serverVariables, setServerVariables] = useState<Record<string, ServerConfigVariable[]>>({});
  const [serverInstructions, setServerInstructions] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    selectedServers.forEach(server => {
      ...
      const envVars: ServerConfigVariable[] = Object.entries(server.env || {}).map(([name, value]) => ({
        name,
        value: value || '',
    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1 h-[calc(100%-4rem)] overflow-auto">
        <div className="space-y-6">
          ...
          {selectedServers.length > 0 && selectedServers.filter(server => (serverVariables[server.id]?.length || 0) > 0).length !== 0 && (
            <div className="space-y-8">
              {selectedServers
                .filter(server => (serverVariables[server.id]?.length || 0) > 0)
                .map((server) => (
                <div key={server.id} className="border-b pb-6">
                  ...
                  <div className="pl-1">
                    ...
                    <div className="space-y-5">
                      {serverVariables[server.id]?.map((variable, index) => (
