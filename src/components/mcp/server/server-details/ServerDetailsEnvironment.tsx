          ...
          {envPairs.map((pair, index) => (
            <div key={index} className="flex gap-2 group">
              <Input
                ...
                value={pair.key}
                onChange={(e) => updateEnvPair && updateEnvPair(index, 'key', e.target.value)}
                placeholder={t('serverDetails.key')}
              />
              <Input
                className="w-3/5 font-mono group-hover:border-primary/50 transition-colors"
                value={pair.value}
                onChange={(e) => updateEnvPair && updateEnvPair(index, 'value', e.target.value)}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Settings className="h-4 w-4 text-muted-foreground" />
        <h3 className="text-sm font-medium text-primary">{t('serverDetails.environmentVariables')}</h3>
        {Object.keys(server.env || {}).length > 0 && (
          <Badge variant="outline" className="ml-2 text-xs">{Object.keys(server.env || {}).length}</Badge>
        )}
      </div>
      <div className="pl-6">
        {Object.keys(server.env || {}).length > 0 ? (
              ...
              <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm font-mono text-primary/90">
                {Object.entries(server.env || {}).map(([key, value], i) => (
                  <React.Fragment key={i}>
                    ...
                    <div className="opacity-90 overflow-hidden break-all whitespace-pre-wrap">{String(value)}</div>
                  </React.Fragment>
                ))}
              </div>
            </ScrollArea>
