...
import { LocalMCPServer, MCPServerConfig, PaginatedResponse, MCPServer, APIMCPServer } from '../../../types';
...
const DiscoverWrapper: React.FC = () => {
  ...
  const [isLoadingVerifiedServers, setIsLoadingVerifiedServers] = useState(false);
  const [isLoadingCommunityServers, setIsLoadingCommunityServers] = useState(false);
  ...
  const [debouncedVerifiedSearchTerm, setDebouncedVerifiedSearchTerm] = useState('');
  const [debouncedCommunitySearchTerm, setDebouncedCommunitySearchTerm] = useState('');
  ...
  const convertApiServerToLocalServer = (apiServer: APIMCPServer): LocalMCPServer => {
    return {
      id: apiServer.id,
  ...
  const fetchVerifiedServers = async (page = verifiedCurrentPage, limit = verifiedPageLimit, search = debouncedVerifiedSearchTerm) => {
    // Fetch verified servers
    setIsLoadingVerifiedServers(true);
    try {
      const verifiedResponse: PaginatedResponse<APIMCPServer> = 
        await window.electronAPI.fetchMcpServersFromIndex(page, limit, search, true);
      // Convert API servers to LocalMCPServer format
      const localServers = verifiedResponse.data.map(convertApiServerToLocalServer);
  ...
  const fetchCommunityServers = async (page = communityCurrentPage, limit = communityPageLimit, search = debouncedCommunitySearchTerm) => {
    // Fetch community servers
    setIsLoadingCommunityServers(true);
    try {
      const communityResponse: PaginatedResponse<APIMCPServer> = 
        await window.electronAPI.fetchMcpServersFromIndex(page, limit, search, false);
      // Convert API servers to LocalMCPServer format
      const localServers = communityResponse.data.map(convertApiServerToLocalServer);
  ...
  const fetchInstalledServers = async () => {
    try {
      ...
      setInstalledServerIds(new Set(servers.map((server: MCPServer) => server.id)));
    } catch (error) {
  ...
  const handleVerifiedPageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= verifiedTotalPages) {
      ...
      fetchVerifiedServers(newPage, verifiedPageLimit, debouncedVerifiedSearchTerm);
    }
  };
  ...
  const handleCommunityPageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= communityTotalPages) {
      ...
      fetchCommunityServers(newPage, communityPageLimit, debouncedCommunitySearchTerm);
    }
  };
  ...
  const handleImportServer = async (server: LocalMCPServer) => {
    ...
    try {
      ...
      if (server.displayId && server.latestVersion) {
        versionDetails = await window.electronAPI.fetchMcpServerVersionDetails(server.displayId, server.latestVersion);
      }
      ...
      if (versionDetails && versionDetails.version) {
        ...
        if (versionDetails.version.verificationStatus) {
          serverConfig.verificationStatus = versionDetails.version.verificationStatus;
        ...
        if (server.verificationStatus) serverConfig.verificationStatus = server.verificationStatus;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="manual" className="space-y-6">
        <div>
          <h2 className="text-lg font-bold mb-4 text-center">{t('discoverServers.title')}</h2>
          <TabsList className="w-full border-b">
            <TabsTrigger value="registry" className="flex-1">{t('discoverServers.tabs.registry')}</TabsTrigger>
            <TabsTrigger value="manual" className="flex-1">{t('discoverServers.tabs.manual')}</TabsTrigger>
          </TabsList>
        </div>
        ...
        <TabsContent value="registry" className="space-y-6">
          <Tabs defaultValue="verified" className="space-y-4">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="verified">{t('discoverServers.verifiedTab')}</TabsTrigger>
              <TabsTrigger value="community">{t('discoverServers.communityTab')}</TabsTrigger>
            </TabsList>
