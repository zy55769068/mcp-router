...
import { RequestLogEntry, TimeSeriesDataPoint } from '../../../../lib/types/log-types';
 ...
 */
export const useTimeSeriesData = (params: TimeSeriesParams): TimeSeriesResult => {
   ...
   * ログデータを時系列データに変換する関数
   */
  function convertLogsToTimeSeries(logs: RequestLogEntry[]): TimeSeriesDataPoint[] {
    ...
    logs.forEach(log => {
      ...
      // カウントを増やす
      clientMapEntry.set(log.requestType, clientMapEntry.get(log.requestType)! + 1);
    });
    ...
    sortedTimeBuckets.forEach(timeBucket => {
      ...
      const timestamp = dayjs.tz(`${datePart} ${hourPart}:00:00`, 'Asia/Tokyo').valueOf();
