...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getAuthStatus: (forceRefresh?: boolean) => ipcRenderer.invoke('auth:status', forceRefresh),
  handleAuthToken: (token: string, state?: string) => ipcRenderer.invoke('auth:handle-token', token, state),
  ...
  addMcpServer: (serverConfig: MCPServerConfig) => ipcRenderer.invoke('mcp:add', serverConfig),
  removeMcpServer: (id: string) => ipcRenderer.invoke('mcp:remove', id),
  getMcpServerStatus: (id: string) => ipcRenderer.invoke('mcp:status', id),
  updateMcpServerConfig: (id: string, config: any) => ipcRenderer.invoke('mcp:update-config', id, config),
  ...
  getAvailableRequestTypes: () => ipcRenderer.invoke('requestLogs:getRequestTypes'),
  ...
  getRequestTypeStats: () => ipcRenderer.invoke('requestLogs:getRequestTypeStats'),
  ...
  submitInvitationCode: (code: string) => ipcRenderer.invoke('activation:submit', code),
  
  // Settings Management
  getSettings: () => ipcRenderer.invoke('settings:get'),
  saveSettings: (settings: any) => ipcRenderer.invoke('settings:save', settings),
  incrementPackageManagerOverlayCount: () => ipcRenderer.invoke('settings:increment-package-manager-overlay-count'),
  
  // MCP Apps Management
  listMcpApps: () => ipcRenderer.invoke('mcp-apps:list'),
  addMcpAppConfig: (appName: string) => ipcRenderer.invoke('mcp-apps:add', appName),
  deleteMcpApp: (appName: string) => ipcRenderer.invoke('mcp-apps:delete', appName),
  updateAppServerAccess: (appName: string, serverIds: string[]) => 
    ipcRenderer.invoke('mcp-apps:update-server-access', appName, serverIds),
  unifyAppConfig: (appName: string) => ipcRenderer.invoke('mcp-apps:unify', appName),

  
  // Agent Management
  listAgents: () => ipcRenderer.invoke('agent:list'),
  getAgent: (id: string) => ipcRenderer.invoke('agent:get', id),
  createAgent: (agentConfig: any) => ipcRenderer.invoke('agent:create', agentConfig),
  updateAgent: (id: string, config: any) => ipcRenderer.invoke('agent:update', id, config),
  deleteAgent: (id: string) => ipcRenderer.invoke('agent:delete', id),
  shareAgent: (id: string) => ipcRenderer.invoke('agent:share', id),
  importAgent: (shareCode: string) => ipcRenderer.invoke('agent:import', shareCode),
  ...
  getDeployedAgent: (id: string) => ipcRenderer.invoke('agent:deployed-get', id),
  ...
  startBackgroundChat: (sessionId: string | undefined, agentId: string, query: string) => 
    ipcRenderer.invoke('agent:background-chat-start', sessionId, agentId, query),
  ...
  // Command check
  checkCommandExists: (command: string) => ipcRenderer.invoke('command:exists', command),
  
  // Token Scope Management
  updateTokenScopes: (tokenId: string, scopes: TokenScope[]) => ipcRenderer.invoke('token:updateScopes', tokenId, scopes),

  // Feedback
  submitFeedback: (feedback: string) => ipcRenderer.invoke('feedback:submit', feedback),
