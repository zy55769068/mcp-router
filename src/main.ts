import { app, autoUpdater, BrowserWindow, ipcMain, session, shell } from 'electron';
...
import { fetchMcpServersFromIndex, fetchMcpServerVersionDetails } from './main/mcp-fetcher';
 ...
 */
function setupTrayUpdateTimer(mcpServerManager: MCPServerManager, intervalMs = 5000) {
  if (trayUpdateTimer) {
...
function setupMcpServerHandlers(): void {
    ...
    try {
      ...
      if (serverConfig.serverType !== 'local') {
        ...
        return {
          success,
          server,
          message: success ? 'Remote server connected successfully' : 'Failed to connect to remote server'
        };
      }
  ...
  ipcMain.handle('mcp:update-config', (_, id: string, config: Partial<MCPServerConfig>) => {
    const result = mcpServerManager.updateServer(id, config);
    return result;
  });

  ipcMain.handle('mcp:fetch-from-index', async (_, page?: number, limit?: number, search?: string, isVerified?: boolean) => {
    return await fetchMcpServersFromIndex(page, limit, search, isVerified);
  });

  ipcMain.handle('mcp:fetch-server-version-details', async (_, displayId: string, version: string) => {
...
function setupLogHandlers(): void {
  ipcMain.handle('requestLogs:get', async (_, options?: {
    clientId?: string;
    serverId?: string;
    requestType?: string;
    startDate?: Date;
  ...
  ipcMain.handle('requestLogs:getRequestTypeStats', async () => {
    try {
      ...
      console.error('[RequestLogs] Error retrieving request type stats:', error);
      return [];
    }
  });
}
...
function setupSettingsHandlers(): void {
  ...
  ipcMain.handle('settings:increment-package-manager-overlay-count', () => {
    try {
      ...
      const newCount = (currentSettings.packageManagerOverlayDisplayCount || 0) + 1;
...
function setupMcpAppsHandlers(): void {
  ...
  ipcMain.handle('mcp-apps:update-server-access', async (_, appName: string, serverIds: string[]) => {
    try {
...
function setupTokenHandlers(): void {
  ipcMain.handle('token:updateScopes', (_, tokenId: string, scopes: TokenScope[]) => {
    try {
