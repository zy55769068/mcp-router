...
interface AgentState {
  ...
  // Operations
  createDevelopmentAgent: (config: Omit<AgentConfig, 'id' | 'createdAt' | 'updatedAt'>) => Promise<AgentConfig>;
  updateDevelopmentAgentConfig: (id: string, config: Partial<AgentConfig>) => Promise<void>;
  ...
  fetchChatSessions: (agentId: string, cursor?: string, append?: boolean) => Promise<void>;
...
export const useAgentStore = create<AgentState>((set, get) => ({
  ...
  removeDevelopmentAgent: (id) =>
    set((state) => ({
      developmentAgents: state.developmentAgents.filter((agent) => agent.id !== id),
      currentDevelopmentAgent:
        state.currentDevelopmentAgent?.id === id ? null : state.currentDevelopmentAgent,
    })),
  
  setCurrentDevelopmentAgent: (currentDevelopmentAgent) => set({ currentDevelopmentAgent }),
  ...
  removeDeployedAgent: (id) =>
    set((state) => ({
      ...
      currentDeployedAgent:
        state.currentDeployedAgent?.id === id ? null : state.currentDeployedAgent,
    })),
  
  setCurrentDeployedAgent: (currentDeployedAgent) => set({ currentDeployedAgent }),
  ...
  setCurrentStreamMessage: (currentStreamMessage) => set({ currentStreamMessage }),
  
  setIsStreaming: (isStreaming) => set({ isStreaming }),
  
  resetChatState: (_agentId, instructions = '') => set({
    messages: [
      {
        id: 'system-1',
        role: 'system',
  ...
  setLoadingMoreSessions: (isLoadingMoreSessions) => set({ isLoadingMoreSessions }),
  ...
  deployAgent: async (agentId) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to deploy agent');
      throw error;
    }
  },
  ...
  undeployAgent: async (agentId) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to undeploy agent');
      throw error;
    }
  },

  fetchChatSessions: async (agentId, cursor, append = false) => {
    const { setLoadingSessions, setLoadingMoreSessions, setSessionsError, chatSessions } = get();
  ...
  refreshAgents: async () => {
    const { setLoading, setError, setDevelopmentAgents, setDeployedAgents } = get();
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to refresh agents');
    } finally {
      setLoading(false);
    }
  },
...
export const useCurrentAgent = () =>
  useAgentStore((state) => state.currentDevelopmentAgent || state.currentDeployedAgent);
