  ...
  try {
    ...
    if (responseString.length > MAX_RESPONSE_SIZE) {
      ...
      if (typeof response === 'object') {
        ...
        const collectionTypes = ['tools', 'resources', 'resourceTemplates', 'prompts', 'contents'];
        
        for (const collectionType of collectionTypes) {
          if (collectionType in response && Array.isArray(response[collectionType])) {
        ...
        for (const key of Object.keys(response)) {
          if (!collectionTypes.includes(key)) {
            ...
            if (Array.isArray(value)) {
              sampleData[key] = value.length > 0 ? 
                { type: 'array', length: value.length, sample: value.slice(0, 2) } : 
                { type: 'array', length: 0 };
      ...
      return {
        ...
        _sample: typeof response === 'string' ? response.substring(0, 1000) + '...' : response,
        _fullDataAvailable: false
      };
    }
