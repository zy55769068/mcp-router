  ...
  if (os.platform() === 'win32') {
    await run(
        ...
        [
          ...
          '-Command', 'iwr https://astral.sh/uv/install.ps1 -useb | iex'
        ]
    );
  } else {
...
export async function installPNPM() {
  ...
  if (!nodeExists) {
    try {
      ...
      throw new Error('pnpm was installed but Node.js installation failed. You may need to install Node.js manually.');
    }
  }
}
