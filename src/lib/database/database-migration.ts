...
export class DatabaseMigration {
  ...
  private registerMigrations(): void {
    ...
    this.migrations.push({
      ...
      description: "Manage agent tables: drop for reinitialization and add auto_execute_tool column to deployedAgents",
    ...
    this.migrations.push({
      ...
      description: "Add scopes column to tokens table and populate with default scopes",
  ...
  public runMigrations(): void {
    try {
      ...
      for (const migration of this.migrations) {
        ...
        console.log(`マイグレーション ${migration.id} を実行中: ${migration.description}`);
  ...
  private migrateAddServerTypeColumn(db: SqliteManager): void {
    try {
      ...
      if (!tableExists) {
        console.log('serversテーブルが存在しないため、このマイグレーションをスキップします');
        return;
      }
      ...
      if (!columnNames.includes('server_type')) {
        ...
        db.execute("ALTER TABLE servers ADD COLUMN server_type TEXT NOT NULL DEFAULT 'local'");
  ...
  private migrateAddVerificationStatusColumn(db: SqliteManager): void {
    try {
      ...
      if (!columnNames.includes('verification_status')) {
        ...
        console.log('verification_status列は既に存在するため、追加をスキップします');
      }
    } catch (error) {
      console.error('verification_status列の追加中にエラーが発生しました:', error);
      throw error;
    }
  }
  ...
  private migrateAddRequiredParamsColumn(db: SqliteManager): void {
    try {
      ...
      if (!columnNames.includes('required_params')) {
        ...
        console.log('required_params列は既に存在するため、追加をスキップします');
      }
    } catch (error) {
  ...
  private migrateTokensAddScopes(db: SqliteManager): void {
    try {
      ...
      if (!tableExists) {
        console.log('tokensテーブルが存在しないため、このマイグレーションをスキップします');
        return;
      }
      ...
      console.error('トークンテーブルのスコープカラム追加中にエラーが発生しました:', error);
      throw error;
    }
  }
  ...
  private migrateAgentTableManagement(db: SqliteManager): void {
    try {
      ...
      if (agentsTableExists || deployedAgentsTableExists) {
        ...
        console.log('エージェントテーブルの削除が完了しました。次回のアプリケーション起動時にAgentRepositoryによって再作成されます。');
      } else {
        console.log('エージェントテーブルが存在しないため、削除処理をスキップします');
      }
      ...
      console.error('エージェントテーブル管理マイグレーション中にエラーが発生しました:', error);
      throw error;
    }
  }
  ...
  private migrateAddOriginalIdToDeployedAgents(db: SqliteManager): void {
    try {
      ...
      if (!columnNames.includes('original_id')) {
        ...
        db.execute("ALTER TABLE deployedAgents ADD COLUMN original_id TEXT NOT NULL DEFAULT ''");
  ...
  private migrateToEncryption(db: SqliteManager): void {
    try {
      if (!safeStorage.isEncryptionAvailable()) {
        console.warn('セキュア暗号化は現在のシステムで利用できません。データ移行をスキップします。');
        return;
      }
      ...
      if (allServers.length === 0) {
        console.log('サーバーが存在しないため、暗号化マイグレーションをスキップします');
        return;
      }
      ...
      for (const server of allServers) {
        try {
          ...
          console.error(`サーバー "${server.name}" (ID: ${server.id}) の暗号化に失敗しました:`, error);
        }
      }
      
      console.log(`${migratedCount}個のサーバーデータを暗号化しました`);
    } catch (error) {
      console.error('サーバーデータの暗号化移行中にエラーが発生しました:', error);
