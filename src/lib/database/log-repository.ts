...
export class LogRepository extends BaseRepository<RequestLogEntry> {
  ...
  protected initializeTable(): void {
    try {
      ...
      // インデックスの作成
      this.db.execute(`CREATE INDEX IF NOT EXISTS idx_logs_timestamp ON ${this.tableName} (timestamp)`);
      this.db.execute(`CREATE INDEX IF NOT EXISTS idx_logs_client_id ON ${this.tableName} (client_id)`);
      this.db.execute(`CREATE INDEX IF NOT EXISTS idx_logs_server_id ON ${this.tableName} (server_id)`);
      this.db.execute(`CREATE INDEX IF NOT EXISTS idx_logs_request_type ON ${this.tableName} (request_type)`);
      this.db.execute(`CREATE INDEX IF NOT EXISTS idx_logs_response_status ON ${this.tableName} (response_status)`);
      ...
      const metadata = this.db.get<{count: number}>('SELECT COUNT(*) as count FROM logs_metadata WHERE key = :key', { key: 'client_ids' });
      if (!metadata || metadata.count === 0) {
        this.db.execute('INSERT INTO logs_metadata (key, value) VALUES (:key, :value)', { key: 'client_ids', value: '[]' });
        this.db.execute('INSERT INTO logs_metadata (key, value) VALUES (:key, :value)', { key: 'server_ids', value: '[]' });
        this.db.execute('INSERT INTO logs_metadata (key, value) VALUES (:key, :value)', { key: 'request_types', value: '[]' });
        this.db.execute('INSERT INTO logs_metadata (key, value) VALUES (:key, :value)', { key: 'last_cleanup', value: String(Date.now()) });
      }
    } catch (error) {
      console.error('リクエストログテーブルの初期化中にエラーが発生しました:', error);
      throw error;
    }
  }
  ...
  private updateMetadataArray(key: string, value: string): void {
    // 現在の値を取得
    const metadataRow = this.db.get<{value: string}>('SELECT value FROM logs_metadata WHERE key = :key', { key });
    if (!metadataRow) {
  ...
  public getAvailableClientIds(): string[] {
    try {
      const metadata = this.db.get<{value: string}>('SELECT value FROM logs_metadata WHERE key = :key', { key: 'client_ids' });
      return metadata ? JSON.parse(metadata.value) : [];
    } catch (error) {
      console.error('クライアントIDリストの取得中にエラーが発生しました:', error);
      return [];
    }
  }
  ...
  public getAvailableRequestTypes(): string[] {
    try {
      const metadata = this.db.get<{value: string}>('SELECT value FROM logs_metadata WHERE key = :key', { key: 'request_types' });
      return metadata ? JSON.parse(metadata.value) : [];
    } catch (error) {
      console.error('リクエストタイプリストの取得中にエラーが発生しました:', error);
      return [];
    }
  }
  ...
  public getClientStats(): ClientStats[] {
    try {
      ...
      const rows = this.db.all<{client_id: string, client_name: string, request_count: number}>(sql);
      ...
      console.error('クライアント統計情報の取得中にエラーが発生しました:', error);
      return [];
    }
  }
  ...
  public getServerStats(): ServerStats[] {
    try {
      ...
      const rows = this.db.all<{server_id: string, server_name: string, request_count: number}>(sql);
  ...
  public getRequestTypeStats(): RequestTypeStats[] {
    try {
      ...
      console.error('リクエストタイプ統計情報の取得中にエラーが発生しました:', error);
      return [];
    }
  }
