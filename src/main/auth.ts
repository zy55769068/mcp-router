...
import {decryptStringSync, encryptStringSync} from '../lib/utils/encryption-utils';
...
export function startAuthFlow(idp?: string) {
      ...
      authUrl.searchParams.append('callbackUrl', callbackUrl.pathname + callbackUrl.search);
 ...
 */
async function exchangeCodeForToken(code: string, codeVerifier: string, idp?: string) {
  try {
 ...
 */
export async function status(forceRefresh = false): Promise<{ authenticated: boolean, userId?: string, user?: any, token?: string }> {
  try {
    const settingsService = getSettingsService();
    const settings = settingsService.getSettings();
    ...
    if (!forceRefresh && userDataCache && (Date.now() - userDataCache.lastFetched < USER_DATA_CACHE_TTL)) {
