 ...
 */
export async function syncServersFromClientConfig(serverConfigs: MCPServerConfig[]): Promise<void> {
...
export async function importExistingServerConfigurations(): Promise<void> {
  try {
    console.log('Checking for existing MCP server configurations in client apps...');
    ...
    for (const config of clientConfigs) {
      ...
      const serverConfigs = extractServerConfigs(config.type, config.content, config.path);
      ...
      for (const serverConfig of serverConfigs) {
        ...
        try {
          ...
          console.error(`Failed to import server '${serverConfig.name}' from ${config.type}:`, error);
        }
      }
    }
  } catch (error) {
    ...
    try {
      if (await exists(config.path)) {
        ...
        try {
          ...
          console.error(`Failed to parse ${config.type} config file:`, parseError);
        }
      }
    } catch (error) {
 ...
 */
export async function extractConfigInfo(name: string, configPath: string): Promise<{
 ...
 */
export function extractServersFromConfig(servers: Record<string, any>): MCPServerConfig[] {
 ...
 */
function extractServerConfigs(clientType: ClientType, content: any, configPath: string): MCPServerConfig[] {
  const configs: MCPServerConfig[] = [];
    ...
    switch (clientType) {
      ...
      case 'cursor':
        ...
        if (content.mcpServers) {
          extractStandardServerConfigs(content.mcpServers, configs, clientType, configPath);
        }
        break;
    }
  } catch (error) {
 ...
 */
function extractVSCodeServerConfigs(servers: Record<string, any>, configs: MCPServerConfig[], clientType: ClientType): void {
  for (const [serverName, serverConfig] of Object.entries(servers)) {
    if (serverConfig && typeof serverConfig === 'object') {
 ...
 */
function extractStandardServerConfigs(servers: Record<string, any>, configs: MCPServerConfig[], clientType: ClientType, configPath: string): void {
  for (const [serverName, serverConfig] of Object.entries(servers)) {
    if (serverConfig && typeof serverConfig === 'object') {
      // Skip 'mcp-router' server as it's this app itself
      if (serverName === 'mcp-router') continue;
