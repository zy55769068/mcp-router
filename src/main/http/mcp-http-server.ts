...
import { listMcpApps } from '../../lib/services/mcp-apps-service';
...
export class MCPHttpServer {
  ...
  private configureMiddleware(): void {
    ...
    const authMiddleware = (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      // Validate the token
      const tokenId = typeof token === 'string' ? (token.startsWith('Bearer ') ? token.substring(7) : token) : '';
      ...
      if (requiredScope && !this.tokenService.hasScope(tokenId, requiredScope)) {
        res.status(403).json({
  ...
  private configureMcpRoute(): void {
    ...
    this.app.post("/mcp", async (req, res) => {
      ...
      try {
        await this.serverManager.getTransport().handleRequest(req, res, modifiedBody);
      } catch (error) {
  ...
  private configureAppsRoutes(): void {
    ...
    this.v0Router.get('/apps', async (req: express.Request, res: express.Response) => {
      try {
  ...
  private configureLogRoutes(): void {
    ...
    this.v0Router.get('/logs', async (req: express.Request, res: express.Response) => {
      try {
        const options: {
          clientId?: string;
          serverId?: string;
          ...
          responseStatus: req.query.responseStatus as 'success' | 'error' | undefined,
    ...
    this.v0Router.get('/logs/stats', (req: express.Request, res: express.Response) => {
      try {
    ...
    this.v0Router.get('/logs/clients', (req: express.Request, res: express.Response) => {
      try {
    ...
    this.v0Router.get('/logs/types', (req: express.Request, res: express.Response) => {
      try {
    ...
    this.v0Router.get('/logs/:id', (req: express.Request, res: express.Response) => {
      try {
  ...
  private configureAggregatorRoutes(): void {
    ...
    this.v0Router.post('/servers/:id/start', async (req: express.Request, res: express.Response) => {
      try {
        ...
        if (token) {
          ...
          const extractedClientId = this.tokenService.getClientIdFromToken(token);
        ...
        const success = await this.serverManager.startServer(serverId, clientId);
    ...
    this.v0Router.post('/servers/:id/stop', async (req: express.Request, res: express.Response) => {
      try {
        const serverId = req.params.id;
    ...
    this.v0Router.delete('/servers/:id', async (req: express.Request, res: express.Response) => {
      try {
        const serverId = req.params.id;
        ...
        res.status(500).json({
          error: {
            ...
            message: error.message || 'An unexpected error occurred while removing the server'
          }
        });
      }
    });
    ...
    this.v0Router.get('/servers', (req: express.Request, res: express.Response) => {
      try {
        const servers = this.serverManager.getServers();
    ...
    this.v0Router.post('/servers', async (req: express.Request, res: express.Response) => {
      try {
        ...
        if (!validation.valid || !validation.serverConfigs) {
          res.status(400).json({
            error: {
              ...
              message: validation.error || 'Missing or invalid server configuration'
            }
          });
          return;
        }
        ...
        const existingServerNames = new Set<string>(existingServers.map(server => server.name));

        // Process server configurations using the utility function
        const processedResults = processMcpServerConfigs(validation.serverConfigs, existingServerNames);
