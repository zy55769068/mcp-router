...
import { TokenGenerateOptions, TokenScope } from './lib/types/token-types';
import { AppSettings } from './lib/types/settings-types';
import { Agent, AgentConfig, DeployedAgent, MCPServerConfig } from './types';
import {McpAppsManagerResult, McpApp} from "@/lib/services/mcp-apps-service";
import { ServerPackageUpdates } from './lib/utils/package-version-resolver';
...
declare global {
  interface Window {
    electronAPI: {
      ...
      getAuthStatus: (forceRefresh?: boolean) => Promise<{ authenticated: boolean; userId?: string; user?: any, token?: string }>;
      handleAuthToken: (token: string, state?: string) => Promise<boolean>;
      onAuthStatusChanged: (callback: (status: { loggedIn: boolean; userId?: string; user?: any }) => void) => () => void;
      ...
      fetchMcpServersFromIndex: (page?: number, limit?: number, search?: string, isVerified?: boolean) => Promise<any>;
      ...
      completeAgentSetup: (id: string, completed: boolean, updatedServers?: any[]) => Promise<Agent | undefined>;
      ...
      // Background Chat
      startBackgroundChat: (sessionId: string | undefined, agentId: string, query: string) => Promise<{success: boolean, error?: string}>;
      stopBackgroundChat: (agentId: string) => Promise<{success: boolean, error?: string}>;
      ...
      sendChatStreamError: (errorData: any) => Promise<{success: boolean, error?: string}>;
      ...
      checkPackageManagers: () => Promise<{ node: boolean; pnpm: boolean; uv: boolean }>;
      installPackageManagers: () => Promise<{ success: boolean; installed: { node: boolean; pnpm: boolean; uv: boolean }; errors?: { node?: string; pnpm?: string; uv?: string } }>;
