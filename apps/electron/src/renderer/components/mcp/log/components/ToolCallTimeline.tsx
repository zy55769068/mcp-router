...
const ToolCallTimeline: React.FC<ToolCallTimelineProps> = ({
  ...
  const groupedToolCalls = useMemo(() => {
    ...
    const byDate = toolCalls.reduce<Record<string, RequestLogEntry[]>>(
      (acc, log) => {
    ...
    return Object.entries(byDate)
      .map(([date, dateLogs]) => {
        ...
        const byInterval = dateLogs.reduce<Record<string, RequestLogEntry[]>>(
          (acc, log) => {
