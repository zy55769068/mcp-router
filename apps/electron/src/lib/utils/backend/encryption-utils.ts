 ...
 */
export async function setEncryptionInitialized(
  initialized: boolean,
): Promise<boolean> {
...
function initializeMasterKey(): void {
  ...
  try {
    ...
    if (encryptedMasterKeyBase64) {
      // ― 既存キーを復号 ―
      const encryptedMasterKey = Buffer.from(
        encryptedMasterKeyBase64,
        "base64",
      );
      masterKey = Buffer.from(
        safeStorage.decryptString(encryptedMasterKey),
        "hex",
      );
    } else {
      // ― 新規生成 ―
      masterKey = crypto.randomBytes(MASTER_KEY_SIZE);
      const encryptedMasterKey = safeStorage.encryptString(
