...
export function setupAgentHandlers(): void {
  ...
  ipcMain.handle("agent:get", (_: IpcMainInvokeEvent, id: string) => {
    return developmentAgentService.getAgentById(id);
  });

  ipcMain.handle(
    "agent:create",
    (_: IpcMainInvokeEvent, agentConfig: Omit<AgentConfig, "id">) => {
  ...
  ipcMain.handle(
    "agent:update",
    (_: IpcMainInvokeEvent, id: string, config: Partial<AgentConfig>) => {
      return developmentAgentService.updateAgent(id, config);
    },
  );

  ipcMain.handle("agent:delete", (_: IpcMainInvokeEvent, id: string) => {
    return developmentAgentService.deleteAgent(id);
  });

  // 共有関連のIPC通信ハンドラ
  ipcMain.handle("agent:share", async (_: IpcMainInvokeEvent, id: string) => {
  ...
  ipcMain.handle(
    "agent:import",
    async (_: IpcMainInvokeEvent, shareCode: string) => {
      const sharedData =
        await agentSharingService.getSharedAgentData(shareCode);
      const deployedAgentData =
        agentSharingService.convertToDeployedAgent(sharedData);
      const result =
        deployedAgentService.createDeployedAgent(deployedAgentData);
      return result;
    },
  );

  // エージェントデプロイ関連のIPC通信ハンドラ
  ipcMain.handle("agent:deploy", (_: IpcMainInvokeEvent, id: string) => {
  ...
  ipcMain.handle(
    "agent:deployed-update",
    (_: IpcMainInvokeEvent, id: string, config: any) => {
  ...
  ipcMain.handle(
    "agent:background-chat-stop",
    async (_: IpcMainInvokeEvent, agentId: string) => {
      try {
        if (!backgroundWindow) {
          throw new Error("Background window not available");
        }
  ...
  ipcMain.handle(
    ...
    async (_: IpcMainInvokeEvent, streamData: any) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_: IpcMainInvokeEvent, chunkData: any) => {
      try {
  ...
  ipcMain.handle(
    "agent:chat-stream-end",
    async (_: IpcMainInvokeEvent, endData: any) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_: IpcMainInvokeEvent, errorData: any) => {
      try {
  ...
  ipcMain.handle(
    "agent:get-sessions",
    async (_: IpcMainInvokeEvent, agentId: string, options: any = {}) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_: IpcMainInvokeEvent, sessionId: string, messages: any[]) => {
      try {
