    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "Query to send to the agent (will be processed asynchronously)",
        },
      },
      required: ["query"],
    },
...
export class AgentToolHandler {
  private static _deployedAgentService: ReturnType<
    typeof getDeployedAgentService
  > | null = null;
  private static _sessionRepository: ReturnType<
    typeof getSessionRepository
  > | null = null;
  ...
  public static getEnabledAgentTools(): AgentTool[] {
    try {
      ...
      const agentTools = agents
        ...
        .map((agent) =>
          createAgentTool(agent.name, agent.description, agent.purpose),
        );
      ...
      const queryResultTools: AgentTool[] = [
        {
          ...
          description:
            "Retrieve the result of a specific agent query by session ID. Use this to get the response from a previously submitted agent query.",
          inputSchema: {
            type: "object",
            properties: {
              sessionId: {
                type: "string",
                description:
                  "The session ID returned when the query was submitted",
              },
            },
            required: ["sessionId"],
          },
        },
        {
          ...
          description:
            "List agent sessions. Without filters, returns the most recent sessions sorted by last update time. Can be filtered by status.",
          inputSchema: {
            type: "object",
            properties: {
              status: {
  ...
  private static async handleCallAgent(args: any): Promise<any> {
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to call agent: ${error.message || "Unknown error"}`,
      );
    }
  }
    ...
    try {
      ...
      if (session.status === "completed" && session.messages.length > 0) {
        ...
        if (assistantMessages.length > 0) {
          ...
          resultContent =
            lastAssistantMessage.content || "No content in response";
        } else {
...
${
  session.status === "completed"
    ...
    : session.status === "failed"
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get query result: ${error.message || "Unknown error"}`,
      );
    }
  }
  ...
  private static async handleListSessions(args: any): Promise<any> {
    ...
    try {
      ...
      const sessionList = sessions.sessions.map((session) => ({
        sessionId: session.id,
